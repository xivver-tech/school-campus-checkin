#!/usr/bin/env python3
"""School Campus Check-In - Rowad Nahda Private (Tiznit) - FR/AR/EN"""
...
body{font-family:system-ui,-apple-system,Tahoma,sans-serif;background:var(--bg);color:var(--text);min-height:100dvh;padding-bottom:calc(72px + var(--safe))}
...
.status-in{color:var(--ok);font-weight:800;font-size:1.2rem}.status-out{color:var(--muted);font-weight:800;font-size:1.2rem}.status-late{color:var(--bad);font-weight:700}
...
.bottom-nav a{flex:1;text-align:center;text-decoration:none;color:var(--muted);font-size:.68rem;padding:.35rem;font-weight:600}
.bottom-nav a.active{color:var(--accent)}.bottom-nav .ico{font-size:1.2rem;display:block}
...
@media(min-width:700px){body{padding-bottom:1rem}.bottom-nav{display:none}header nav.desk{display:flex}header nav.desk a{color:var(--accent);text-decoration:none;margin-inline-start:.75rem;font-size:.85rem}}
@media(max-width:699px){header nav.desk{display:none}.grid2{grid-template-columns:1fr}}
</style></head><body>
<header>
<div class="brand">{{ school_name }}</div>
<div class="lang">
<a href="/lang/fr">FR</a><a href="/lang/ar">AR</a><a href="/lang/en">EN</a>
</div>
<nav class="desk">{% if user %}
<a href="{{ url_for('dashboard') }}">{{ tr.home }}</a>
<a href="{{ url_for('history') }}">{{ tr.history }}</a>
{% if user.role in ['admin','teacher'] %}<a href="{{ url_for('campus_board') }}">{{ tr.campus }}</a><a href="{{ url_for('report') }}">{{ tr.report }}</a>{% endif %}
...
<a href="{{ url_for('report') }}"><span class="ico">📊</span>{{ tr.report }}</a>
{% endif %}
{% if user.role=='admin' %}<a href="{{ url_for('admin') }}"><span class="ico">⚙️</span>{{ tr.admin }}</a>
{% else %}<a href="{{ url_for('logout') }}"><span class="ico">🚪</span>{{ tr.logout }}</a>{% endif %}
</nav>{% endif %}
...
def api_check():
    ...
    get_db().commit()
    if etype=="in":
        msg=f"IN {now[11:16]}. "+(tr("marked_late") if late else tr("welcome"))
    else:
        msg=f"OUT {now[11:16]}. "+tr("good_day")
...
def admin_body():
    ...
    <p class="muted" style="margin-top:.5rem">{tr("gate_tip")}</p></div>
...
if __name__=="__main__":
    init_db()
    print("Rowad Nahda Private - Tiznit | FR / AR / EN")
